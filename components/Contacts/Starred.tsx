import React from "react";
import styles from "../../styles/starred.module.css";
import { FiArrowLeft } from "react-icons/fi";

const Starred = () => {
  return (
    <div className={styles.container}>
      <div className={styles.header}>
        <FiArrowLeft />
        <div>Starred Messages</div>
        <div>&nbsp;</div>
      </div>
      <div className={styles.body}>
        <div className={styles.message}>
          <div className={styles.msg_header}>
            <img className={styles.profile_img} src="portitem1.jpeg" alt="" />
            <p>Kevin Mitaki</p>
            <p>Test 15</p>
          </div>
          <div className={styles.msg_body}>
            Lorem ipsum dolor sit amet, consectetur adipisicing elit. Similique
            odio dolor in, nesciunt officia dolores quaerat! Earum voluptatem
            fugiat ut aliquid fugit, sint blanditiis quibusdam accusantium minus
            et pariatur dignissimos?
          </div>
        </div>
        <div className={styles.message}>
          <div className={styles.msg_header}>
            <img className={styles.profile_img} src="portitem1.jpeg" alt="" />
            <p>Kevin Mitaki</p>
            <p>Test 15</p>
          </div>
          <div className={styles.msg_body}>
            Lorem ipsum dolor sit amet, consectetur adipisicing elit. Similique
            odio dolor in, nesciunt officia dolores quaerat! Earum voluptatem
            fugiat ut aliquid fugit, sint blanditiis quibusdam accusantium minus
            et pariatur dignissimos?
          </div>
        </div>
        <div className={styles.message}>
          <div className={styles.msg_header}>
            <img className={styles.profile_img} src="portitem1.jpeg" alt="" />
            <p>Kevin Mitaki</p>
            <p>Test 15</p>
          </div>
          <div className={styles.msg_body}>
            Lorem ipsum dolor sit amet, consectetur adipisicing elit. Similique
            odio dolor in, nesciunt officia dolores quaerat! Earum voluptatem
            fugiat ut aliquid fugit, sint blanditiis quibusdam accusantium minus
            et pariatur dignissimos?
          </div>
        </div>
        <div className={styles.message}>
          <div className={styles.msg_header}>
            <img className={styles.profile_img} src="portitem1.jpeg" alt="" />
            <p>Kevin Mitaki</p>
            <p>Test 15</p>
          </div>
          <div className={styles.msg_body}>
            Lorem ipsum dolor sit amet, consectetur adipisicing elit. Similique
            odio dolor in, nesciunt officia dolores quaerat! Earum voluptatem
            fugiat ut aliquid fugit, sint blanditiis quibusdam accusantium minus
            et pariatur dignissimos?
          </div>
        </div>
        <div className={styles.message}>
          <div className={styles.msg_header}>
            <img className={styles.profile_img} src="portitem1.jpeg" alt="" />
            <p>Kevin Mitaki</p>
            <p>Test 15</p>
          </div>
          <div className={styles.msg_body}>
            Lorem ipsum dolor sit amet, consectetur adipisicing elit. Similique
            odio dolor in, nesciunt officia dolores quaerat! Earum voluptatem
            fugiat ut aliquid fugit, sint blanditiis quibusdam accusantium minus
            et pariatur dignissimos?
          </div>
        </div>
        <div className={styles.message}>
          <div className={styles.msg_header}>
            <img className={styles.profile_img} src="portitem1.jpeg" alt="" />
            <p>Kevin Mitaki</p>
            <p>Test 15</p>
          </div>
          <div className={styles.msg_body}>
            Lorem ipsum dolor sit amet, consectetur adipisicing elit. Similique
            odio dolor in, nesciunt officia dolores quaerat! Earum voluptatem
            fugiat ut aliquid fugit, sint blanditiis quibusdam accusantium minus
            et pariatur dignissimos?
          </div>
        </div>
        <div className={styles.message}>
          <div className={styles.msg_header}>
            <img className={styles.profile_img} src="portitem1.jpeg" alt="" />
            <p>Kevin Mitaki</p>
            <p>Test 15</p>
          </div>
          <div className={styles.msg_body}>
            Lorem ipsum dolor sit amet, consectetur adipisicing elit. Similique
            odio dolor in, nesciunt officia dolores quaerat! Earum voluptatem
            fugiat ut aliquid fugit, sint blanditiis quibusdam accusantium minus
            et pariatur dignissimos?
          </div>
        </div>
        <div className={styles.message}>
          <div className={styles.msg_header}>
            <img className={styles.profile_img} src="portitem1.jpeg" alt="" />
            <p>Kevin Mitaki</p>
            <p>Test 15</p>
          </div>
          <div className={styles.msg_body}>
            Lorem ipsum dolor sit amet, consectetur adipisicing elit. Similique
            odio dolor in, nesciunt officia dolores quaerat! Earum voluptatem
            fugiat ut aliquid fugit, sint blanditiis quibusdam accusantium minus
            et pariatur dignissimos?
          </div>
        </div>
        <div className={styles.message}>
          <div className={styles.msg_header}>
            <img className={styles.profile_img} src="portitem1.jpeg" alt="" />
            <p>Kevin Mitaki</p>
            <p>Test 15</p>
          </div>
          <div className={styles.msg_body}>
            Lorem ipsum dolor sit amet, consectetur adipisicing elit. Similique
            odio dolor in, nesciunt officia dolores quaerat! Earum voluptatem
            fugiat ut aliquid fugit, sint blanditiis quibusdam accusantium minus
            et pariatur dignissimos?
          </div>
        </div>
      </div>
    </div>
  );
};

export default Starred;
